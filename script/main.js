...
function start(){
	...
	if(val[val.indexOf(".") + 1] === undefined){
	...
	if(from === to || val === "0"){
		result.innerHTML = `<span>${val}</span><sub>${to}</sub>`;
		return 0;
	};
...
inp.addEventListener("keydown", function(e) {
	...
	if(e.ctrlKey && !e.shiftKey && e.key === "Enter"){
		if($(".table__end").children("span").text() !== ""){
			navigator.clipboard.writeText($(".table__end").children("span").text());

			result.classList.add("copy-block_copied");
			clearTimeout(copyTimeout);
			copyTimeout = setTimeout(function(){
				result.classList.remove("copy-block_copied");
			},300);
		};
	};
...
$("input").on("paste", function(e){
	...
	if(this.classList.contains("counting__ss")){
		if((this.value + pastedData).length > maxSS){
			e.preventDefault();
			error([`Итоговое значение <b>${this.value + pastedData}</b> превышает допустимый предел длины <b>${maxSS}</b>`], false);
			return 0;
		};
...
function errorShow(text, deleteVal){
	document.querySelector(".alert__text").innerHTML = "";
	for(let i = 0; i < text.length; i++){
		document.querySelector(".alert__text").innerHTML += `<div class="alert__text-item">${text[i]}</div>`
	};
	$('.alert').addClass("alert_alerted");
	$('.alert__button').focus();

	clearTimeout(alertTimeout);
	alertTimeout = setTimeout(function(){
		$(".alert").removeClass("alert_alerted");
	},40 * 1000);
...
function error(text = ["Произошла ошибка"], deleteVal = true){
	if(!$(".alert").hasClass("alert_alerted")){
		errorShow(text, deleteVal);
	} else {
		clearTimeout(alertTimeout);
		$(".alert").removeClass("alert_alerted");
		alertTimeout = setTimeout(function(){
...
function toAnother(val, from, to){
	let res = "", num = +val;
	if(from !== 10){
		num = +to10(val, from);
	};
...
$(".table__out-block .system__button").on("click", function(){
	$(".to").removeClass("to");
	$(this).addClass("to");
});
...
function lockKey(e, length, maxLength, symbols){
	if(!keyboardChars.includes(e.key.toLowerCase()) &&
		!e.ctrlKey && e.key.toLowerCase !== "c"){
		let leng = length >= maxLength;
		if(symbols){
			if(leng ||
				!symbolChars.includes(e.key.toLowerCase()) && !trueChars.includes(e.key.toLowerCase())){
				e.preventDefault();
			};
		} else {
			if(leng || !trueChars.includes(e.key.toLowerCase())){
				e.preventDefault();
			};
		};
	};
...
$("#act").on("keydown", function(e){
	if(!keyboardChars.includes(e.key.toLowerCase())){
		if(this.value.length + 1 === 2){
			e.preventDefault();
		} else if (!actionChars.includes(e.key) && e.key !== "Shift"){
			e.preventDefault();
		};
	};
});
...
function charLength(str, char = "", strong){
	let leng = 0;
...
function decimal(num, ss){
	...
	if(!systemValid[0]){
		error([`Ошибка соответсвия числа <b>${num}</b> и его системы счистления на <b>${systemValid[1] + 1}-ом</b> символе`])
		return 0;
	};
	...
	for(let i = num.indexOf(".") + 1; i < num.length; i++){
		res += toNum(num[i]) * ss**(-level);
		level++;
	};
...
function decimalOut(num, to){
	...
	if(typeof num !== "number" || typeof to !== "number"){
		num = +num;
		to = +to;
	};
	...
	if(Math.floor(num) === 0){
		...
		res = toAnother(Math.floor(num), 10, to) + ".";
		num -= Math.floor(num);
		num = toFixed(num, 10);
	};
	...
	do {
		num 	= num * to;
		res 	+= toChar(Math.floor(num));
		num 	-= Math.floor(num)

		limitLength++;
	} while (num !== 0 && limitLength < 12);
...
function count(){
	let
		ss 		= document.getElementById("main-system").value,
		num1		= document.getElementById("num_1").value,
		num2		= document.getElementById("num_2").value,
		act 		= document.getElementById("act").value,
		ss1 		= document.getElementById("ss_1").value,
		ss2 		= document.getElementById("ss_2").value,

		valid1 	= checkSystem(num1, ss1),
		valid2 	= checkSystem(num2, ss2),
		errorMessage = [];

	if(!ss){errorMessage.push("Введите систему счистления результата.");};
	if(!num1){errorMessage.push("Введите значение <b>1-ого</b> числа.");};
	if(!num2){errorMessage.push("Введите значение <b>2-ого</b> числа.");};
	...
	if(num1[num1.indexOf(".") + 1] === undefined){
		num1 = num1.substr(0, num1.indexOf("."));
	};
	if(num2[num2.indexOf(".") + 1] === undefined){
		num2 = num2.substr(0, num2.indexOf("."));
	};

	if(num1.indexOf(".") !== -1){
		num1In10 = decimal(num1, ss1);
	} else if(ss1 !== 10)
		num1In10 = to10(num1 + "", ss1);

	if(num2.indexOf(".") !== -1){
		num2In10 = decimal(num2, ss2);
	} else if(ss2 !== 10)
		num2In10 = to10(num2, ss2);

	if (act === "+"){
		res = num1In10 + num2In10;
	} else if (act === "-"){
		res = num1In10 - num2In10;
	} else if (act === "*" || act === "×" || act === "⋅"){
		if(num1In10 !== 0 && num2In10 !== 0)
			res = num1In10 * num2In10;
		else
			res = 0;
	} else if (act === "/" || act === ":"){
	...
	if(+ss !== 10 && res !== 0){
		...
		if((res + "").indexOf(".") !== -1){
			res = decimalOut(res, ss);
		} else {
			res = toAnother(res, 10, ss);
		};
...
// for(let i = 0; i < document.getElementsByClassName("input-count").length; i++){
// 	$(".input-count").eq("keyup", function(e){

// 	});
// };

$(".input-count, .counting__ss").on("keyup", function(e){
	if(e.key === "Enter" && !e.ctrlKey){
		count();
	};
	if(e.ctrlKey && !e.shiftKey && e.key === "Enter"){
		if($("#count-value").children("span").text() !== ""){
			navigator.clipboard.writeText($("#count-value").children("span").text());

			$("#count-value").addClass("copy-block_copied");
			clearTimeout(copyTimeout);
			copyTimeout = setTimeout(function(){
				$("#count-value").removeClass("copy-block_copied");
			},300);
		};
	};
